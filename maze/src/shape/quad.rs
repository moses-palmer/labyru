use crate::matrix;
use crate::physical;
use crate::wall;

use crate::WallPos;
use crate::wall::{Angle, Offset};

use super::{COS_45, SIN_45};

/// A span step angle
///
/// This is half the angle span used by a single wall.
const D: f32 = 2.0 * std::f32::consts::PI / 8.0;

/// The scale factor when converting maze coordinates to physical coordinates
const MULTIPLICATOR: f32 = 2.0 / std::f32::consts::SQRT_2;

define_shape! {
    << Quad >>

    UP(1) = {
        corner_wall_offsets: &[
            Offset { dx: 0, dy: -1, wall: &LEFT  },
            Offset { dx: -1, dy: -1, wall: &DOWN  },
            Offset { dx: -1, dy: 0, wall: &RIGHT  },
        ],
        dir: (0, -1),
        span: (
            Angle {
                a: 5.0 * D,
                dx: -COS_45,
                dy: -SIN_45,
            },
            Angle {
                a: 7.0 * D,
                dx: COS_45,
                dy: -SIN_45,
            },
        ),
        previous: &LEFT,
        next: &RIGHT,
    },
    LEFT(0) = {
        corner_wall_offsets: &[
            Offset { dx: -1, dy: 0, wall: &DOWN  },
            Offset { dx: -1, dy: 1, wall: &RIGHT  },
            Offset { dx: 0, dy: 1, wall: &UP  },
        ],
        dir: (-1, 0),
        span: (
            Angle {
                a: 3.0 * D,
                dx: -COS_45,
                dy: SIN_45,
            },
            Angle {
                a: 5.0 * D,
                dx: -COS_45,
                dy: -SIN_45,
            },
        ),
        previous: &DOWN,
        next: &UP,
    },
    DOWN(3) = {
        corner_wall_offsets: &[
            Offset { dx: 0, dy: 1, wall: &RIGHT  },
            Offset { dx: 1, dy: 1, wall: &UP  },
            Offset { dx: 1, dy: 0, wall: &LEFT  },
        ],
        dir: (0, 1),
        span: (
            Angle {
                a: D,
                dx: COS_45,
                dy: SIN_45,
            },
            Angle {
                a: 3.0 * D,
                dx: -COS_45,
                dy: SIN_45,
            },
        ),
        previous: &RIGHT,
        next: &LEFT,
    },
    RIGHT(2) = {
        corner_wall_offsets: &[
            Offset { dx: 1, dy: 0, wall: &UP  },
            Offset { dx: 1, dy: -1, wall: &LEFT  },
            Offset { dx: 0, dy: -1, wall: &DOWN  },
        ],
        dir: (1, 0),
        span: (
            Angle {
                a: 7.0 * D,
                dx: COS_45,
                dy: -SIN_45,
            },
            Angle {
                a: 1.0 * D,
                dx: COS_45,
                dy: SIN_45,
            },
        ),
        previous: &UP,
        next: &DOWN,
    }
}

/// The walls
static WALLS: &[&wall::Wall] = &[&walls::LEFT, &walls::UP, &walls::RIGHT, &walls::DOWN];

pub fn minimal_dimensions(width: f32, height: f32) -> (usize, usize) {
    let height = (height.max(MULTIPLICATOR) / MULTIPLICATOR).ceil() as usize;

    let width = (width.max(MULTIPLICATOR) / MULTIPLICATOR).ceil() as usize;

    (width, height)
}

pub fn back_index(wall: usize) -> usize {
    wall ^ 0b0010
}

pub fn opposite(wall_pos: WallPos) -> Option<&'static wall::Wall> {
    let wall = wall_pos.wall;
    Some(walls::ALL[(wall.index + walls::ALL.len() / 2) % walls::ALL.len()])
}

pub fn walls(_pos: matrix::Pos) -> &'static [&'static wall::Wall] {
    WALLS
}

pub fn cell_to_physical(pos: matrix::Pos) -> physical::Pos {
    physical::Pos {
        x: (pos.col as f32 + 0.5) * MULTIPLICATOR,
        y: (pos.row as f32 + 0.5) * MULTIPLICATOR,
    }
}

pub fn physical_to_cell(pos: physical::Pos) -> matrix::Pos {
    matrix::Pos {
        col: (pos.x / MULTIPLICATOR).floor() as isize,
        row: (pos.y / MULTIPLICATOR).floor() as isize,
    }
}

#[allow(clippy::collapsible_else_if)]
pub fn physical_to_wall_pos(pos: physical::Pos) -> WallPos {
    let matrix_pos = physical_to_cell(pos);
    let center = cell_to_physical(matrix_pos);
    let (dx, dy) = (pos.x - center.x, pos.y - center.y);

    let wall = if dx > dy {
        if dy > -dx { &walls::RIGHT } else { &walls::UP }
    } else {
        if dy > -dx { &walls::DOWN } else { &walls::LEFT }
    };

    (matrix_pos, wall).into()
}

#[cfg(test)]
mod tests {
    use maze_test::maze_test;

    use super::*;
    use crate::WallPos;
    use crate::test_utils::*;

    #[maze_test(quad)]
    fn back(maze: TestMaze) {
        assert_eq!(
            maze.back((matrix_pos(1, 1), &walls::LEFT).into()),
            (matrix_pos(0, 1), &walls::RIGHT).into()
        );
        assert_eq!(
            maze.back((matrix_pos(1, 1), &walls::UP).into()),
            (matrix_pos(1, 0), &walls::DOWN).into()
        );
        assert_eq!(
            maze.back((matrix_pos(1, 1), &walls::RIGHT).into()),
            (matrix_pos(2, 1), &walls::LEFT).into()
        );
        assert_eq!(
            maze.back((matrix_pos(1, 1), &walls::DOWN).into()),
            (matrix_pos(1, 2), &walls::UP).into()
        );
    }

    #[maze_test(quad)]
    fn opposite(maze: TestMaze) {
        assert_eq!(
            maze.opposite((matrix_pos(1, 1), &walls::LEFT).into())
                .unwrap(),
            &walls::RIGHT
        );
        assert_eq!(
            maze.opposite((matrix_pos(1, 1), &walls::UP).into())
                .unwrap(),
            &walls::DOWN
        );
        assert_eq!(
            maze.opposite((matrix_pos(1, 1), &walls::RIGHT).into())
                .unwrap(),
            &walls::LEFT
        );
        assert_eq!(
            maze.opposite((matrix_pos(1, 1), &walls::DOWN).into())
                .unwrap(),
            &walls::UP
        );
    }

    #[maze_test(quad)]
    fn corner_walls(maze: TestMaze) {
        assert_eq!(
            maze.corner_walls_start((matrix_pos(1, 1), &walls::UP).into())
                .collect::<Vec<_>>(),
            vec![
                (matrix_pos(1, 1), &walls::UP).into(),
                (matrix_pos(1, 0), &walls::LEFT).into(),
                (matrix_pos(0, 0), &walls::DOWN).into(),
                (matrix_pos(0, 1), &walls::RIGHT).into(),
            ],
        );

        assert_eq!(
            maze.corner_walls_start((matrix_pos(1, 1), &walls::LEFT).into())
                .collect::<Vec<_>>(),
            vec![
                (matrix_pos(1, 1), &walls::LEFT).into(),
                (matrix_pos(0, 1), &walls::DOWN).into(),
                (matrix_pos(0, 2), &walls::RIGHT).into(),
                (matrix_pos(1, 2), &walls::UP).into(),
            ],
        );

        assert_eq!(
            maze.corner_walls_start((matrix_pos(1, 1), &walls::DOWN).into())
                .collect::<Vec<_>>(),
            vec![
                (matrix_pos(1, 1), &walls::DOWN).into(),
                (matrix_pos(1, 2), &walls::RIGHT).into(),
                (matrix_pos(2, 2), &walls::UP).into(),
                (matrix_pos(2, 1), &walls::LEFT).into(),
            ],
        );

        assert_eq!(
            maze.corner_walls_start((matrix_pos(1, 1), &walls::RIGHT).into())
                .collect::<Vec<_>>(),
            vec![
                (matrix_pos(1, 1), &walls::RIGHT).into(),
                (matrix_pos(2, 1), &walls::UP).into(),
                (matrix_pos(2, 0), &walls::LEFT).into(),
                (matrix_pos(1, 0), &walls::DOWN).into(),
            ],
        );
    }

    #[maze_test(quad)]
    fn follow_no_wall(mut maze: TestMaze) {
        let log = Navigator::new(&mut maze).down(true).stop();

        let start_pos = (*log.first().unwrap(), &walls::DOWN).into();
        let expected = vec![];
        assert_eq!(maze.follow_wall(start_pos).collect::<Vec<_>>(), expected);
    }

    #[maze_test(quad)]
    fn follow_wall_single_room(maze: TestMaze) {
        assert_eq!(
            vec![
                (matrix_pos(0, 0), &walls::LEFT).into(),
                (matrix_pos(0, 0), &walls::UP).into(),
                (matrix_pos(0, 0), &walls::RIGHT).into(),
                (matrix_pos(0, 0), &walls::DOWN).into(),
            ] as Vec<WallPos>,
            maze.follow_wall((matrix_pos(0, 0), &walls::LEFT).into())
                .map(|(from, _)| from)
                .collect::<Vec<WallPos>>()
        );
    }

    #[maze_test(quad)]
    fn follow_wall(mut maze: TestMaze) {
        Navigator::new(&mut maze)
            .from(matrix_pos(0, 0))
            .down(true)
            .right(true)
            .up(true);

        assert_eq!(
            vec![
                (matrix_pos(0, 0), &walls::LEFT).into(),
                (matrix_pos(0, 0), &walls::UP).into(),
                (matrix_pos(0, 0), &walls::RIGHT).into(),
                (matrix_pos(1, 0), &walls::LEFT).into(),
                (matrix_pos(1, 0), &walls::UP).into(),
                (matrix_pos(1, 0), &walls::RIGHT).into(),
                (matrix_pos(1, 1), &walls::RIGHT).into(),
                (matrix_pos(1, 1), &walls::DOWN).into(),
                (matrix_pos(0, 1), &walls::DOWN).into(),
                (matrix_pos(0, 1), &walls::LEFT).into(),
            ] as Vec<WallPos>,
            maze.follow_wall((matrix_pos(0, 0), &walls::LEFT).into())
                .map(|(from, _)| from)
                .collect::<Vec<WallPos>>()
        );
    }
}
