...
///
/// See [here](https://en.wikipedia.org/wiki/Maze_generation_algorithm) for a description of the
/// algorithm.
///
    ...
    T: Clone,
{
    ...
    loop {
        ...
        // Find all non-visited neighbours as the tuple (neighbour-position, wall-from-current)
        let neighbors = maze
            .walls(current)
            .iter()
        ...
        // previous room; since the maze may be segmented, we must also attempt to find a new
        // random room
